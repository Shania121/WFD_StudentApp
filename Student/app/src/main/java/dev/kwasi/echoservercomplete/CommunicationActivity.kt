...
class CommunicationActivity : AppCompatActivity(), WifiDirectInterface, PeerListAdapterInterface, NetworkMessageInterface {
    ...
    public var studentIdText: String? = null
    ...
    fun discoverNearbyPeers(view: View) {
        //Checking if valid student id was inputted
        val studentId: EditText = findViewById(R.id.enteredStudentId)
        val range = 816000000..816999999
        val studentIdText = studentId.text.toString()
        if (range.contains(studentIdText.toInt())){
    ...
    private fun updateUI(){
       ...
       // val nearbyTextBox:ConstraintLayout= findViewById(R.id.nearbyText)
        //nearbyTextBox.visibility = if (wfdAdapterEnabled && !wfdHasConnection && hasDevices) View.VISIBLE else View.GONE
    ...
    override fun onGroupStatusChanged(groupInfo: WifiP2pGroup?) {
        ...
        if (groupInfo == null){
            ...
            client = Client(this)
            deviceIp = client!!.ip
            val enteredStudentId: EditText = findViewById(R.id.enteredStudentId)
        }
    }
